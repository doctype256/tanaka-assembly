      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', boxShadow: '0 2px 5px rgba(0,0,0,0.1)' }}>
        <thead>
          <tr style={{ backgroundColor: '#2d3748', color: 'white', textAlign: 'left' }}>
            ...
            <th style={thStyle}>相談内容（クリックで詳細）</th>
          ...
          {filteredData.map((item) => (
            <tr 
              ...
              style={{ 
                ...
                backgroundColor: item.needs_reply && item.status !== 'completed' ? '#fffaf0' : '#fff' 
              }}
            >
              <td style={tdStyle}>
          ...
          <div style={modalContentStyle} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ marginTop: '20px' }}>
              <div style={modalSectionStyle}>
                <strong>送信日時:</strong> {new Date(selectedItem.created_at).toLocaleString()}
              </div>
              <div style={modalSectionStyle}>
                <strong>分類:</strong> {selectedItem.target_type} ＞ {selectedItem.place_type} ＞ {selectedItem.content_type}
              </div>
              <div style={modalSectionStyle}>
                <strong>メールアドレス:</strong> {selectedItem.needs_reply ? (
                  <a href={`mailto:${selectedItem.email}`} style={{ color: '#3182ce' }}>{selectedItem.email} (クリックでメール作成)</a>
                ) : '不要'}
              </div>
              <div style={modalSectionStyle}>
                <strong>相談内容:</strong>
                <div style={messageBoxStyle}>{selectedItem.message}</div>
              </div>
            </div>
          </div>
        </div>
