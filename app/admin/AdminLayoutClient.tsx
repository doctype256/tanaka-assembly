...
interface AdminLayoutClientProps {
  ...
  initialUserName: string; // 🟢 サーバーから受け取る名前
}
...
export default function AdminLayoutClient({ children, initialUserName }: AdminLayoutClientProps) {
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] flex text-[#020617] overflow-x-hidden font-sans">
      {/* モバイルメニューボタン */}
      <button onClick={() => setIsMenuOpen(!isMenuOpen)} className="fixed top-5 left-5 z-[60] p-3 bg-[#020617] rounded-xl md:hidden text-white border border-slate-700">
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-72 bg-[#020617] flex flex-col transform transition-transform duration-300 md:relative md:translate-x-0 ${isMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="p-10 flex-shrink-0">
          <div className="flex items-center gap-3 mb-2 text-white">
            <div className="w-8 h-8 bg-[#EAB308] rounded-lg flex items-center justify-center text-black shadow-[0_0_15px_rgba(234,179,8,0.3)]"><HiHome size={18} /></div>
        ...
        <div className="p-6 border-t border-slate-800/50 bg-slate-900/30">
          <div className="flex items-center gap-3 px-2">
            ...
            <div className="flex flex-col min-w-0">
              ...
              <p className="text-xs font-black text-slate-200 truncate">{initialUserName}</p>
            </div>
          </div>
        </div>
      </aside>
      ...
      {isMenuOpen && <div className="fixed inset-0 bg-[#020617]/90 backdrop-blur-md z-40 md:hidden" onClick={() => setIsMenuOpen(false)} />}
    </div>
  );
}